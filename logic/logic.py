...
def get_machines(tokens) -> list:
    ...
    machinesList = dfMach[['id', 'principalId', 'model.name', 'serialNumber', 'name', 'organization.id']].values.tolist()
...
def heatmap(window, df_load_profile, model, pin, iso_datetime_from, iso_datetime_to):
    ...
    if window.plot_ref is None:
        window.plot_ref = 1
    else:
        ax.clear()

    im = canvas.axes.imshow(data, cmap=plt.cm.YlGn, aspect='auto')
    ...
    date_from = f"{iso_datetime_from[8:-14]}/{iso_datetime_from[5:-17]}/{iso_datetime_from[0:-20]}"
    date_to = f"{iso_datetime_to[8:-14]}/{iso_datetime_to[5:-17]}/{iso_datetime_to[0:-20]}"
